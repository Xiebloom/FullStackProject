...
var pg = require('pg'); //var funcs = require('./funcs');
//DB 定義
...
router.post('/create', function (req, res, next) {
  ...
  var createId;
  var date = req.body['userDate']; //随机生成实验数据

  var starttime = new Array();
  var finishtime = new Array();
  generatedata(starttime, finishtime);
  var slen = starttime.length; //新しいユーザーを登録

  pgPool.connect(function (err, client) {
    if (err) {
      console.log(err);
    } else {
      console.log('User Resgiter!'); //まず、最新のユーザーIDを抽出

      var query = {
        text: 'SELECT NAME,id FROM public."TEST_TBL" ORDER BY id DESC LIMIT 1' //values: [userName,id],
      ...
      .query(query, function (err, result) {
        if (err) {
          ...
          console.log('finding user data...'); //若找到了确实存在的数据

          if (result.rows.length) {
            console.log('user data Found.');
            userName = result.rows[0].name; //　console.log(userName);

            console.log('最新のユーザーは');
            console.log(result.rows[0].id); //新ID生成

            createId = setnewID(result.rows[0].id);
            console.log('new ID is ' + createId); //将新用户名和ID写入 用户名&ID 数据库

            var query = {
              text: 'INSERT INTO public."TEST_TBL" (name,id) VALUES($1,$2)',
              values: [name, createId]
            };
            client.query(query).then(function () {//res.send("Data Created.");
            })["catch"](function (e) {
            ...
            for (var k = 1; k <= slen; k++) {
              var query = {
                text: 'INSERT INTO public."EXAMDATA" (userid,TimeOrder,StartTime,FinishTime) VALUES($1,$2,$3,$4)',
...
router.post('/select', function (req, res, next) {
  //こっちの　/select　は 相対パス、Renderのところは pug の名前
  ...
  console.log('userID = ' + id); //  传送给DB的text，结合2个表，找出开始时间，结束时间，用户ID，用户名   

  var querysql = {
    text: //'SELECT NAME FROM public."TEST_TBL" WHERE id = $1 ',
    'SELECT starttime,finishtime,id,name FROM public."TEST_TBL" INNER JOIN public."EXAMDATA" ON public."TEST_TBL".id = public."EXAMDATA".userid  WHERE userid = $1',
    //sqlのテーブル結合
    values: [id] //  values: [foundstarttime,foundfinishtime,id,userName],   //これはDBに渡すデータなんですよ
    ...
    if (err) {
      ...
      .query(querysql, function (err, result) {
        ...
        if (err) {
          ...
          {
            //开始時間，结束时间处理
            console.log('finding time data...'); //console.log(result);
            //若通过用户ID，找到了确实存在的数据　・　IDが確かに存在する場合

            if (result.rows.length) {
              console.log('time data Found.');
              userName = result.rows[0].name;
              id = result.rows[0].id; // console.log(userName);
              //console.log(result);
              ...
              var jsonresult = JSON.stringify(resulttobrowser); //  传送给DB的text，用户ID对应的问卷结果・　当IDのアンケート回答結果

              console.log('find id = ' + id);
              var querysql = {
                text: 'SELECT answer FROM public."ques" WHERE ques_id = $1',
                values: [id]
              };
              client //获取アンケート結果
              .query(querysql, function (err, result) {
                ...
                if (err) {
                ...
                } else {
                  {
                    if (true) {
                      ...
                      console.log(quesans); //传给pug数据・データをpugに渡す
...
function generatedata(starttime, finishtime) {
  ...
  for (var i = 0; i < 20; i++) {
    //保存该回生成的随机数
    r = randomnunber(i, i + 1); //    console.log(r);
    ...
    if (i == 0) {
      ...
      if (r <= finishtime[k]) {
        finishtime[k] = finishtime[k] + randomnunber(0, 1); //        console.log('Finishtime伸びる！' + ' to ' + finishtime[k]);
      } //否则就是新的一段时间 
      else {
          starttime[k + 1] = r;
