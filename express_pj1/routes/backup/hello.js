...
router.post('/create', (req, res, next) => {
  ...
  res.render('hello', data);  //render函数所传递的数据必须是对象，具有｛項目：値...｝的形式。
...
router.post('/select', (req, res, next) => {  //こっちの　/select　は 相対パス、Renderのところは pug の名前
  ...
  pgPool.connect(function (err, client) {
    ...
    if (err) {
          ...
          if (err) {
            ...
            {//开始時間，结束时间处理
              ...
              if (result.rows.length) {
                ...
                //console.log('starttime = ' + foundstarttime);
                // console.log('resulttobrowser = ');          
                // console.log(resulttobrowser);    
      ...
      //console.log(data);
      //res.render('select', data);    //ここはだめ、非同期ので、下は先に処理された
    }
  });
});
